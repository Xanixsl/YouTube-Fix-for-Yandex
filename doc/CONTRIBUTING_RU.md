...
> ⚠️ **Важно — ограничения лицензии:** Форк репозитория разрешён **только** для подготовки Pull Request обратно в оригинальный проект. Ваш форк не должен публиковаться, распространяться или поддерживаться как самостоятельный проект. См. [Лицензию](#️-лицензия) ниже.

### Процесс работы

```
1. Сделайте Fork репозитория (только для PR — см. лицензию)
...
7. Удалите форк после мержа или закрытия PR
```
...
- [ ] Форк не будет оставлен как самостоятельное распространение
...
Основные разделы используют заголовок с рамкой:

```js
// ┌──────────────────────────────────────────────────────────────────┐
// │  § N  НАЗВАНИЕ РАЗДЕЛА                                               │
// │  Краткое описание содержимого раздела.                       │
// └──────────────────────────────────────────────────────────────────┘
```

Подразделы внутри основного раздела используют короткие тире:

```js
// --- Название подраздела ---
```

Сохраняйте эти паттерны при добавлении новых разделов — это упрощает навигацию по файлу.

---

## ⚖️ Лицензия

Отправляя Pull Request, вы соглашаетесь с участью вашего вклада на условиях лицензии проекта:

**[Проприетарная лицензия](../LICENSE)** — Copyright (c) 2026 Xanixsl. All Rights Reserved.

Ключевые условия, важные для участников:

| Действие | Разрешено? |
|---------|-------------|
| Личное использование и перераспространение **оригинального** скрипта | ✅ С указанием автора |
| Форк **исключительно** для подготовки Pull Request | ✅ Временно |
| Публикация форка как самостоятельного проекта | ❌ Запрещено (§ 3б) |
| Изменение кода вне подготовки PR | ❌ Запрещено (§ 3а) |
| Создание производных работ, ответвлетвлений, переименованных версий | ❌ Запрещено (§ 3б, 3ф) |
| Коммерческое использование или передача прав | ❌ Запрещено (§ 3г) |

После принятия вклад становится частью ПО и переходит в **исключительную интеллектуальную собственность автора** (Xanixsl) на условиях данной лицензии (§ 4). Вы не сохраняете никаких отдельных прав на переданный код.

Для любого использования за пределами отправки PR запросите письменное разрешение через [GitHub Discussions](https://github.com/Xanixsl/YouTube-Fix-for-Yandex/discussions).
